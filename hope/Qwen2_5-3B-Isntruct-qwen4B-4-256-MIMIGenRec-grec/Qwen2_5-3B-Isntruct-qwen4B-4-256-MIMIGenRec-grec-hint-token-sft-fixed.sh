...
usage() {
  cat <<'EOF'
Usage:
  bash Qwen2_5-3B-Isntruct-qwen4B-4-256-MIMIGenRec-grec-hint-token-sft-fixed.sh [options]

Run modes:
  --nohup                 Export fixed hint map and start masked hint-token SFT in background via nohup (default)
  --detach                Export fixed hint map and start in background without log follow
  --tail                  Follow latest log for current run name
  --run                   Internal mode; execute export + train directly
...
Common overrides:
  ...
  --num-processes <n>
  --port <n>
  --ds-config <path>      Accelerate launcher config. Default: config/zero2.yaml
  --hf-deepspeed-config <path|none>
                          Optional Trainer DeepSpeed JSON. Default: none; accelerate config owns DeepSpeed
  ...
  --grad-acc <n>
  --epochs <n>            Default: 5
  --lr <float>            Default: 3.0e-4, matching the full SFT config
  --hint-sft-loss-coef <float>  Default: 1.0
  --eval-strategy <no|steps>
  --eval-step <n>
  --max-seq-length <n>
  --max-prompt-length <n>
...
OUTPUT_DIR="${OUTPUT_DIR:-${REPO_ROOT}/saves/qwen2.5-3b/full/Instruments-grec-hint-token-sft-fixed-b16-lr3e-4-sft495}"
DS_CONFIG="${DS_CONFIG:-${REPO_ROOT}/config/zero2.yaml}"
HF_DEEPSPEED_CONFIG="${HF_DEEPSPEED_CONFIG:-none}"

NUM_PROCESSES="${NUM_PROCESSES:-4}"
MAIN_PORT="${MAIN_PORT:-29531}"
...
HINT_SFT_LOSS_COEF="${HINT_SFT_LOSS_COEF:-1.0}"
...
RUN_NAME="${RUN_NAME:-instruments_grec_hint_token_sft_fixed_b16_lr3e_4_ckpt495}"
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --run) MODE="run"; shift ;;
    --from-nohup) FROM_NOHUP=1; shift ;;
    --model-path) MODEL_PATH="$2"; shift 2 ;;
    --data-dir) DATA_DIR="$2"; shift 2 ;;
    --index-path) INDEX_PATH="$2"; shift 2 ;;
    --add-tokens-path) ADD_TOKENS_PATH="$2"; shift 2 ;;
    --output-dir) OUTPUT_DIR="$2"; shift 2 ;;
    --resume|--resume-from-checkpoint) RESUME_FROM_CHECKPOINT="$2"; shift 2 ;;
    --num-processes) NUM_PROCESSES="$2"; shift 2 ;;
    --port) MAIN_PORT="$2"; shift 2 ;;
    --ds-config) DS_CONFIG="$2"; shift 2 ;;
    --hf-deepspeed-config) HF_DEEPSPEED_CONFIG="$2"; shift 2 ;;
    --num-beams) NUM_BEAMS="$2"; shift 2 ;;
    --sid-levels) SID_LEVELS="$2"; shift 2 ;;
    --train-bsz) PER_DEVICE_TRAIN_BSZ="$2"; shift 2 ;;
    --eval-bsz) PER_DEVICE_EVAL_BSZ="$2"; shift 2 ;;
    --grad-acc) GRAD_ACC="$2"; shift 2 ;;
    --epochs) NUM_EPOCHS="$2"; shift 2 ;;
    --lr) LEARNING_RATE="$2"; shift 2 ;;
    --hint-sft-loss-coef) HINT_SFT_LOSS_COEF="$2"; shift 2 ;;
    ...
    --analysis-summary-path) ANALYSIS_SUMMARY_PATH="$2"; shift 2 ;;
    --analysis-details-path) ANALYSIS_DETAILS_PATH="$2"; shift 2 ;;
    --fixed-hint-map-path)
      FIXED_HINT_MAP_PATH="$2"
      FIXED_HINT_MAP_PATH_EXPLICIT=1
      shift 2
      ;;
    --beam-size) BEAM_SIZE="$2"; shift 2 ;;
    --unsolved-depth) UNSOLVED_DEPTH="$2"; shift 2 ;;
    --cap) CAP_DEPTH="$2"; shift 2 ;;
    --report-to) REPORT_TO="$2"; shift 2 ;;
    --wandb-mode) export WANDB_MODE="$2"; shift 2 ;;
    --run-name) RUN_NAME="$2"; export WANDB_RUN_NAME="$2"; shift 2 ;;
    --python-bin) PYTHON_BIN="$2"; shift 2 ;;
    --conda-activate) CONDA_ACTIVATE="$2"; shift 2 ;;
    --conda-env) CONDA_ENV_NAME="$2"; shift 2 ;;
    --log-dir) LOG_DIR="$2"; shift 2 ;;
    --log-file) LOG_FILE_OVERRIDE="$2"; shift 2 ;;
...
if [[ "$MODE" == "nohup" || "$MODE" == "detach" ]]; then
  ...
  touch "$LOG_FILE"
  CHILD_ARGS=(
    --run --from-nohup --log-file "$LOG_FILE"
    --model-path "$MODEL_PATH" --data-dir "$DATA_DIR" --index-path "$INDEX_PATH"
    --add-tokens-path "$ADD_TOKENS_PATH" --output-dir "$OUTPUT_DIR"
    --resume "$RESUME_FROM_CHECKPOINT" --num-processes "$NUM_PROCESSES" --port "$MAIN_PORT"
    --ds-config "$DS_CONFIG" --hf-deepspeed-config "$HF_DEEPSPEED_CONFIG"
    --num-beams "$NUM_BEAMS" --sid-levels "$SID_LEVELS"
    --train-bsz "$PER_DEVICE_TRAIN_BSZ" --eval-bsz "$PER_DEVICE_EVAL_BSZ"
    --grad-acc "$GRAD_ACC" --epochs "$NUM_EPOCHS" --lr "$LEARNING_RATE"
    --hint-sft-loss-coef "$HINT_SFT_LOSS_COEF" --eval-strategy "$EVAL_STRATEGY"
    --eval-step "$EVAL_STEP" --max-seq-length "$MAX_SEQ_LENGTH"
    --max-prompt-length "$MAX_PROMPT_LENGTH" --max-completion-length "$MAX_COMPLETION_LENGTH"
    --analysis-summary-path "$ANALYSIS_SUMMARY_PATH" --analysis-details-path "$ANALYSIS_DETAILS_PATH"
    --fixed-hint-map-path "$FIXED_HINT_MAP_PATH" --beam-size "$BEAM_SIZE"
    --unsolved-depth "$UNSOLVED_DEPTH" --report-to "$REPORT_TO" --run-name "$RUN_NAME"
    --python-bin "$PYTHON_BIN" --conda-activate "$CONDA_ACTIVATE" --conda-env "$CONDA_ENV_NAME"
  )
  if [[ -n "$CAP_DEPTH" ]]; then
  ...
  PID=$!
  echo "[INFO] Fixed hint-token SFT pipeline started in background. pid=$PID"
...
require_file "$DS_CONFIG" "accelerate config"
if [[ -n "$HF_DEEPSPEED_CONFIG" ]] && ! is_none_value "$HF_DEEPSPEED_CONFIG"; then
  require_file "$HF_DEEPSPEED_CONFIG" "HF DeepSpeed JSON config"
fi
require_file "${REPO_ROOT}/hint_sft_trainer.py" "hint_sft_trainer.py"

EXPORT_CMD=(
  "$PYTHON_BIN" analyze_rl_beam_hint.py
...
if [[ -n "$HF_DEEPSPEED_CONFIG" ]] && ! is_none_value "$HF_DEEPSPEED_CONFIG"; then
  TRAIN_CMD+=(--deepspeed "$HF_DEEPSPEED_CONFIG")
fi
...
echo "[INFO] HINT_SFT_LOSS_COEF=$HINT_SFT_LOSS_COEF"
echo "[INFO] EFFECTIVE_HINT_LR=$(awk "BEGIN {print ${LEARNING_RATE} * ${HINT_SFT_LOSS_COEF}}")"
echo "[INFO] ACCELERATE_CONFIG=$DS_CONFIG"
echo "[INFO] HF_DEEPSPEED_CONFIG=$HF_DEEPSPEED_CONFIG"
