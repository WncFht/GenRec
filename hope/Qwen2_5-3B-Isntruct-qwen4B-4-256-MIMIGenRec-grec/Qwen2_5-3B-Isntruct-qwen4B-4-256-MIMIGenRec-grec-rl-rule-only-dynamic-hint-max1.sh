...
usage() {
  cat <<'EOF'
Usage:
  bash Qwen2_5-3B-Isntruct-qwen4B-4-256-MIMIGenRec-grec-rl-rule-only-dynamic-hint-max1.sh [options]
...
Common overrides:
  ...
  --beta <float>
  --temperature <float>
  --dynamic-hint-max-depth <n>    Default: 1
...
OUTPUT_DIR="${OUTPUT_DIR:-${REPO_ROOT}/rl_outputs/Instruments-grec-grpo-rule-only-dynamic-hint-max1-qwen2.5-3b-qwen4B-4-256-from-sft495}"
...
DYNAMIC_HINT_MAX_DEPTH="${DYNAMIC_HINT_MAX_DEPTH:-1}"
DYNAMIC_HINT_APPLY_TO_EVAL="${DYNAMIC_HINT_APPLY_TO_EVAL:-false}"
REPORT_TO="${REPORT_TO:-wandb}"
RESUME_FROM_CHECKPOINT="${RESUME_FROM_CHECKPOINT:-auto}"

RUN_NAME="${RUN_NAME:-instruments_grec_rl_rule_only_dynamic_hint_max1_qwen2_5_3b_qwen4b_4_256_from_ckpt495}"
...
if [[ "$MODE" == "nohup" || "$MODE" == "detach" ]]; then
  ...
  PID=$!
  echo "[INFO] Dynamic-hint RL pipeline started in background. pid=$PID"
