...
usage() {
  cat <<'EOF'
Usage:
  bash Qwen2_5-3B-Isntruct-qwen4B-4-256-MIMIGenRec-grec-rl-rule-only-dynamic-hint-sid-hint-only-mixed.sh [options]

Run modes:
  --nohup                 Start mixed-task RL in background via nohup (default)
...
Common overrides:
  ...
  --dynamic-hint-apply-to-eval <true|false>
  --dynamic-hint-task-names <csv> Default: task1_sid_sft
...
OUTPUT_DIR="${OUTPUT_DIR:-${REPO_ROOT}/rl_outputs/Instruments-grec-grpo-rule-only-dynamic-hint-sid-hint-only-mixed-qwen2.5-3b-qwen4B-4-256-from-sft495}"
...
DYNAMIC_HINT_TASK_NAMES="${DYNAMIC_HINT_TASK_NAMES:-task1_sid_sft}"
EVAL_TASK_NAMES="${EVAL_TASK_NAMES:-task1_sid_sft}"
REPORT_TO="${REPORT_TO:-wandb}"
RESUME_FROM_CHECKPOINT="${RESUME_FROM_CHECKPOINT:-auto}"

RUN_NAME="${RUN_NAME:-instruments_grec_rl_rule_only_dynamic_hint_sid_hint_only_mixed_qwen2_5_3b_qwen4b_4_256_from_ckpt495}"
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --dynamic-hint-apply-to-eval)
      ...
      shift 2
      ;;
    --dynamic-hint-task-names|--dynamic_hint_task_names)
...
if [[ "$MODE" == "nohup" || "$MODE" == "detach" ]]; then
  ...
  PID=$!
  echo "[INFO] Dynamic-hint sid-hint-only mixed RL pipeline started in background. pid=$PID"
