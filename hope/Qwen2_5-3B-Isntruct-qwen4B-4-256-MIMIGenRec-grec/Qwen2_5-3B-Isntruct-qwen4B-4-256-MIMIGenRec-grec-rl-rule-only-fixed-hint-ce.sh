...
usage() {
  cat <<'EOF'
Usage:
  bash Qwen2_5-3B-Isntruct-qwen4B-4-256-MIMIGenRec-grec-rl-rule-only-fixed-hint-ce.sh [options]
...
Common overrides:
  ...
  --hint-ce-loss-coef <float>  Default: 0.001
  --run-name <name>
  --python-bin <python>
  --conda-activate <path>
  --conda-env <name>
...
OUTPUT_DIR="${OUTPUT_DIR:-${REPO_ROOT}/rl_outputs/Instruments-grec-grpo-rule-only-fixedhint-taskfix-b16-hintce-sft495}"
...
if [[ "$MODE" == "nohup" || "$MODE" == "detach" ]]; then
  ...
  PID=$!
  echo "[INFO] Fixed-hint CE RL pipeline started in background. pid=$PID"
