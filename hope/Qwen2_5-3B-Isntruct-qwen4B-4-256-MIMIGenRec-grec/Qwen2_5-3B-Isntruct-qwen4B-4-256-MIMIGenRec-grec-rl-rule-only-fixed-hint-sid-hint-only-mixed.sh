...
usage() {
  cat <<'EOF'
Usage:
  bash Qwen2_5-3B-Isntruct-qwen4B-4-256-MIMIGenRec-grec-rl-rule-only-fixed-hint-sid-hint-only-mixed.sh [options]

Run modes:
  --nohup                 Generate sid-only hint analysis if needed and start mixed-task RL in background via nohup (default)
...
Common overrides:
  ...
  --fixed-hint-task-names <csv>  Default: task1_sid_sft
  --eval-task-names <csv>        Default: task1_sid_sft
  --analysis-task-names <csv>    Default: task1_sid_sft
  --run-name <name>
  --python-bin <python>
  --conda-activate <path>
  --conda-env <name>
...
OUTPUT_DIR="${OUTPUT_DIR:-${REPO_ROOT}/rl_outputs/Instruments-grec-grpo-rule-only-fixedhint-taskfix-b16-sid-hint-only-mixed-sft495}"
...
RUN_NAME="${RUN_NAME:-instruments_grec_rl_rule_only_fixed_hint_taskfix_b16_sid_hint_only_mixed_ckpt495}"
...
FIXED_HINT_TASK_NAMES="${FIXED_HINT_TASK_NAMES:-task1_sid_sft}"
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --cap)
      ...
      shift 2
      ;;
    --fixed-hint-task-names|--fixed_hint_task_names)
...
if [[ "$MODE" == "nohup" || "$MODE" == "detach" ]]; then
  ...
  PID=$!
  echo "[INFO] Fixed-hint sid-hint-only mixed RL pipeline started in background. pid=$PID"
...
if [[ ! -f "$ANALYSIS_SUMMARY_PATH" || ! -f "$ANALYSIS_DETAILS_PATH" ]]; then
  echo "[INFO] sid-only-in-mixed hint analysis cache missing, generating summary/details first."
  "${ANALYZE_CMD[@]}"
fi
