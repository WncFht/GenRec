...
usage() {
  cat <<'EOF'
Usage:
  bash Qwen2_5-3B-Isntruct-qwen4B-4-256-MIMIGenRec-grec-rl-rule-only-dynamic-hint-sid-only.sh [options]

Run modes:
  --nohup                 Start RL in background via nohup (default)
  --detach                Start RL in background without log follow
...
DATA_VARIANT_DEFAULT="${DATA_VARIANT_DEFAULT:-Instruments_grec_rlsidonly_index_emb-qwen3-embedding-4B_rq4_cb256-256-256-256_dsInstruments_ridFeb-10-2026-05-40-47}"
...
OUTPUT_DIR="${OUTPUT_DIR:-${REPO_ROOT}/rl_outputs/Instruments-grec-grpo-rule-only-dynamic-hint-sid-only-qwen2.5-3b-qwen4B-4-256-from-sft495}"
...
RUN_NAME="${RUN_NAME:-instruments_grec_rl_rule_only_dynamic_hint_sid_only_qwen2_5_3b_qwen4b_4_256_from_ckpt495}"
...
if [[ "$MODE" == "nohup" || "$MODE" == "detach" ]]; then
  ...
  PID=$!
  echo "[INFO] Dynamic-hint sid-only RL pipeline started in background. pid=$PID"
