...
usage() {
  cat <<'EOF'
Usage:
  bash Qwen2_5-3B-Isntruct-qwen4B-4-256-MIMIGenRec-grec-hint-token-sft-dynamic.sh [options]

Run modes:
  --nohup                 Start dynamic masked hint-token SFT in background via nohup (default)
  --detach                Start in background without log follow
  --tail                  Follow latest log for current run name
  --run                   Internal mode; execute training directly
...
Common overrides:
  ...
  --sid-levels <n>
  --dynamic-hint-max-depth <n> Default: 3
  --train-bsz <n>
  --eval-bsz <n>
  --grad-acc <n>
  --epochs <n>                 Default: 5
...
OUTPUT_DIR="${OUTPUT_DIR:-${REPO_ROOT}/saves/qwen2.5-3b/full/Instruments-grec-hint-token-sft-dynamic-b16-lr3e-4-sft495}"
DS_CONFIG="${DS_CONFIG:-${REPO_ROOT}/config/zero2.yaml}"
HF_DEEPSPEED_CONFIG="${HF_DEEPSPEED_CONFIG:-none}"

NUM_PROCESSES="${NUM_PROCESSES:-4}"
MAIN_PORT="${MAIN_PORT:-29532}"
...
RUN_NAME="${RUN_NAME:-instruments_grec_hint_token_sft_dynamic_b16_lr3e_4_ckpt495}"
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --dynamic-hint-max-depth) DYNAMIC_HINT_MAX_DEPTH="$2"; shift 2 ;;
...
if [[ "$MODE" == "nohup" || "$MODE" == "detach" ]]; then
  ...
  CHILD_ARGS=(
    ...
    --temperature "$TEMPERATURE" --report-to "$REPORT_TO" --run-name "$RUN_NAME"
  ...
  PID=$!
  echo "[INFO] Dynamic hint-token SFT pipeline started in background. pid=$PID"
