...
usage() {
  cat <<'EOF'
Usage:
  bash Qwen2_5-3B-Isntruct-qwen4B-4-256-MIMIGenRec-grec-rl-rule-only-dynamic-hint-ce.sh [options]
...
OUTPUT_DIR="${OUTPUT_DIR:-${REPO_ROOT}/rl_outputs/Instruments-grec-grpo-rule-only-dynamic-hint-cascade-reward-gather-fix-hintce005-qwen2.5-3b-qwen4B-4-256-from-sft495}"
...
RUN_NAME="${RUN_NAME:-instruments_grec_rl_rule_only_dynamic_hint_cascade_reward_gather_fix_hint_ce005_qwen2_5_3b_qwen4b_4_256_from_ckpt495}"
...
if [[ "$MODE" == "nohup" || "$MODE" == "detach" ]]; then
  ...
  PID=$!
  echo "[INFO] Dynamic-hint CE RL pipeline started in background. pid=$PID"
