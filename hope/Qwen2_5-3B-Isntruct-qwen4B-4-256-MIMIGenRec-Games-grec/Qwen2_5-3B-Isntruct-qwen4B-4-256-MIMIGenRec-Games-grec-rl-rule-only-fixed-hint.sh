...
usage() {
  cat <<'EOF'
Usage:
  bash Qwen2_5-3B-Isntruct-qwen4B-4-256-MIMIGenRec-Games-grec-rl-rule-only-fixed-hint.sh [options]

Run modes:
  --nohup                 Export fixed hint map and start RL in background via nohup (default)
  --detach                Export fixed hint map and start RL in background without log follow
...
OUTPUT_DIR="${OUTPUT_DIR:-${REPO_ROOT}/rl_outputs/Games-grec-grpo-rule-only-fixedhint-taskfix-b16-sft495}"
...
RUN_NAME="${RUN_NAME:-games_grec_rl_rule_only_fixed_hint_taskfix_b16_ckpt495}"
ANALYSIS_DIR_DEFAULT="${REPO_ROOT}/temp/rl_beam_hint"
ANALYSIS_SUMMARY_PATH="${ANALYSIS_SUMMARY_PATH:-${ANALYSIS_DIR_DEFAULT}/games_grec_beam_hint_cascade_summary.json}"
ANALYSIS_DETAILS_PATH="${ANALYSIS_DETAILS_PATH:-${ANALYSIS_DIR_DEFAULT}/games_grec_beam_hint_cascade_details.json}"
...
if [[ "$MODE" == "nohup" || "$MODE" == "detach" ]]; then
  ...
  PID=$!
  echo "[INFO] Fixed-hint RL pipeline started in background. pid=$PID"
...
if [[ -f "$ANALYSIS_SUMMARY_PATH" ]]; then
  EXPORT_CMD+=(--reuse-summary-path "$ANALYSIS_SUMMARY_PATH")
fi
if [[ -f "$ANALYSIS_DETAILS_PATH" ]]; then
  EXPORT_CMD+=(--reuse-details-path "$ANALYSIS_DETAILS_PATH")
fi
