...
ROOT_SCRIPT="${SCRIPT_DIR}/../Qwen2_5-3B-Isntruct-qwen4B-4-256-MIMIGenRec-grec/Qwen2_5-3B-Isntruct-qwen4B-4-256-MIMIGenRec-grec-rl-rule-only-fixed-hint-ce.sh"
...
OUTPUT_DIR_DEFAULT="${REPO_ROOT_DEFAULT}/rl_outputs/Instruments-grec-lc4023-fixed-ce"
RUN_NAME_DEFAULT="instruments_grec_lc4023_fixed_ce"
ANALYSIS_RUN_NAME_DEFAULT="instruments_grec_lc4023_fixed_ce_analysis"
HINT_CE_LOSS_COEF_DEFAULT="0.001"
...
exec bash "${ROOT_SCRIPT}" \
  ...
  --analysis-run-name "${ANALYSIS_RUN_NAME:-${ANALYSIS_RUN_NAME_DEFAULT}}" \
  --force-reanalyze \
  --hint-ce-loss-coef "${HINT_CE_LOSS_COEF:-${HINT_CE_LOSS_COEF_DEFAULT}}" \
