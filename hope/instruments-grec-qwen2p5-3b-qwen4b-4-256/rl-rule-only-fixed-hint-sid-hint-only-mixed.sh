...
usage() {
  cat <<'EOF'
Usage:
  bash rl-rule-only-fixed-hint-sid-hint-only-mixed.sh [--ds-config <path>] [--dry-run]
...
OUTPUT_DIR="${REPO_ROOT}/rl_outputs/Instruments-grec-grpo-rule-only-fixedhint-taskfix-b16-sid-hint-only-mixed-sft495"
...
RUN_NAME="instruments_grec_rl_rule_only_fixed_hint_taskfix_b16_sid_hint_only_mixed_ckpt495"
