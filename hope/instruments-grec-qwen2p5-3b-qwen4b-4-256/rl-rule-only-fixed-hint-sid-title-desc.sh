...
usage() {
  cat <<'EOF'
Usage:
  bash rl-rule-only-fixed-hint-sid-title-desc.sh [--ds-config <path>] [--dry-run]
...
OUTPUT_DIR="${REPO_ROOT}/rl_outputs/Instruments-grec-grpo-rule-only-fixedhint-taskfix-b16-sid-title-desc-sft495"
...
RUN_NAME="instruments_grec_rl_rule_only_fixed_hint_taskfix_b16_sid_title_desc_ckpt495"
ANALYSIS_DIR_DEFAULT="${REPO_ROOT}/temp/rl_beam_hint/artifacts"
ANALYSIS_DATASET_ID="$(default_fixed_hint_dataset_id "$DATA_DIR")"
ANALYSIS_TASK_NAMES="task1_sid_sft,task5_title_desc2sid"
ANALYSIS_SCOPE_ID="sid-title-desc"
