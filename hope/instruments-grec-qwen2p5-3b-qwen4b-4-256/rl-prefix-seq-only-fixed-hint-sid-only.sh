...
usage() {
  cat <<'EOF'
Usage:
  bash rl-prefix-seq-only-fixed-hint-sid-only.sh [options]
...
ANALYSIS_DETAILS_PATH="${ANALYSIS_DETAILS_PATH:-${ANALYSIS_DIR_DEFAULT}/instruments_grec_rlsidonly_beam_hint_cascade_details.json}"
...
if [[ ! -f "$ANALYSIS_SUMMARY_PATH" || ! -f "$ANALYSIS_DETAILS_PATH" ]]; then
  echo "[INFO] sid-only hint analysis cache missing, generating summary/details first."
  "${ANALYZE_CMD[@]}"
fi
