...
- `fixed taskfix + CE (hintce-4 / coef=0.01)`
- `dynamic max1`
- `fixed dual-task`
...
- `hintce-4` 当前只同步到 `checkpoint-666`，最好点只有 `NDCG@10=0.0891 / HR@50=0.1873`，明显弱于 `hintce-2/3` 和 plain `fixed taskfix`；按现在的 readout，`0.01` 倍率至少在早期就是偏激且不占优的。
