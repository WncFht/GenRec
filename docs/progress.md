...
- 仓库现在已经支持一条 dual-task filtered setting：
  `task1_sid_sft + task5_title_desc2sid` 参与 train，`task4_hisTitle2sid` 被移除，eval 仍只看 `task1_sid_sft`；dynamic / fixed 两个 launcher 都已落地，但当前本地 `results/` 快照里还没有 dual-task 对应目录。
- mixed-task `single-hint` setting 当前本地快照仍只同步到 `checkpoint-999`：
...
- `single-hint mixed` 相比 full mixed fixed-hint 在前 `333/666/999` checkpoint 的 top-10 略强，但 `HR@50` 略低；
  当前更适合继续作为 early-window strong candidate 跟踪，而不是按已经跑到中段 best 点来解读。
...
- 把 dual-task `sid + title_desc2sid` dynamic / fixed 两条线重新同步出来，再和 `single-hint mixed` 放在同一张图里继续补长。
