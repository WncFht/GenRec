...
\item \texttt{hintce-4}：当前把 top-10 继续往尾段抬高的 `0.01` 倍率 CE 候选。
\end{itemize}
...
fixed CE coef=0.01 & \texttt{checkpoint-2997} & 0.0953 & 0.1204 & 0.1113 & 0.1943 \\
\bottomrule
\end{tabular}
...
\item \texttt{single-hint mixed}、old fixed、clean fixed family、\texttt{hintce-3} 和 \texttt{hintce-4} 已经整体挤进了更右上的一团，因此这几条线才是真正应该继续互相比的“前沿集合”。
\item \texttt{hintce-3} 当前 best 点的 \texttt{HR@50=0.1985} 最强，但它的 best 点更靠中段；\texttt{hintce-4} 则把 top-10 一路抬到了 \texttt{checkpoint-2997}，说明更高 CE 倍率可能在“长跑尾段 top-10”上提供不同形状的收益。
\end{itemize}
...
\item 如果你关心新的最强候选，当前真正需要继续盯的不是单条 fixed，而是 \texttt{single-hint mixed}、\texttt{hintce-3} 和 \texttt{hintce-4}：
  前者更像完整长跑后的主候选，\texttt{hintce-3} 更像中段爆发出的高 coverage / 高 NDCG 候选，\texttt{hintce-4} 则更像把 top-10 延续到后段的高倍率版本。
