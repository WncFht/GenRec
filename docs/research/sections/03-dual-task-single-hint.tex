...
dynamic single-hint mixed & checkpoint-666 & 0.400 & 0.0895 & 0.1864 & 当前已同步到 999，但 best 仍在 666 的 first-look \\
\bottomrule
\end{tabular}
...
\item 新同步的 dynamic single-hint mixed 现在已经补到 \texttt{checkpoint-999}，但 best 仍停在 \texttt{checkpoint-666}：`NDCG@10=0.0895 / HR@50=0.1864`；到 \texttt{checkpoint-999} 时读数是 `0.0893 / 0.1864`。它仍明显高于 dynamic dual-task 的起步段 readout，并且只比同 checkpoint 的 canonical \texttt{dynamic gather-fix} (`0.0907 / 0.1914`) 略低一小截。因此当前更合理的说法依然不是“dynamic single-hint mixed 已经赢了”，而是“它已经有足够像样的早窗信号，值得继续同步后段 checkpoint”。
