...
这一节沿用当前默认口径：每个变体都在其全部已同步 checkpoint 中按 \texttt{NDCG@10} 选出唯一 best checkpoint。 Games 仍保留当前已有的主线五列；Instruments 额外纳入 \texttt{ce0.001 / ce0.01}， Arts 则进一步补入 \texttt{ce0.1}，从而把 overall frontier 一次放全。
...
\toprule
Metric & GenRec(sft) & GenRec(rule) & GenRec(ranking) & GenRec(fixed) & GenRec(fixed + ce0.001) & GenRec(fixed + ce0.005) & GenRec(fixed + ce0.01) & GenRec(fixed + ce0.1) \\
\midrule
HR@1 & 0.0643 & \textbf{0.0743} & 0.0728 & 0.0730 & 0.0728 & 0.0720 & \underline{0.0734} & 0.0702 \\
HR@5 & 0.0949 & 0.1031 & 0.1018 & \underline{0.1041} & 0.1035 & \textbf{0.1048} & 0.1025 & 0.0974 \\
HR@10 & 0.1164 & 0.1215 & 0.1189 & \underline{0.1248} & 0.1239 & \textbf{0.1257} & 0.1216 & 0.1158 \\
HR@20 & 0.1433 & 0.1454 & 0.1412 & 0.1504 & \underline{0.1513} & \textbf{0.1518} & 0.1496 & 0.1405 \\
HR@50 & 0.1941 & 0.1906 & 0.1839 & \underline{0.2007} & \textbf{0.2015} & 0.2006 & 0.1978 & 0.1861 \\
NDCG@5 & 0.0801 & \textbf{0.0893} & 0.0876 & \underline{0.0890} & 0.0885 & 0.0889 & 0.0884 & 0.0843 \\
NDCG@10 & 0.0870 & \underline{0.0952} & 0.0931 & \textbf{0.0956} & 0.0951 & \textbf{0.0956} & 0.0946 & 0.0902 \\
NDCG@20 & 0.0938 & 0.1012 & 0.0987 & \textbf{0.1021} & \underline{0.1020} & \textbf{0.1021} & 0.1016 & 0.0964 \\
NDCG@50 & 0.1038 & 0.1102 & 0.1071 & \textbf{0.1120} & \underline{0.1119} & 0.1118 & 0.1111 & 0.1054 \\
\midrule
Selected ckpt & \texttt{checkpoint-522} & \texttt{checkpoint-2105} & \texttt{checkpoint-1684} & \texttt{checkpoint-3368} & \texttt{checkpoint-4206} & \texttt{checkpoint-2105} & \texttt{checkpoint-4206} & \texttt{checkpoint-2105} \\
\bottomrule
\end{tabular}%
}
...
这一节把消融分成两层：训练任务范围，以及 loss 设计。前者只比较当前已经跑完的 three-way fixed prefix variants；后者则把 \texttt{fixed}、\texttt{fixed+CE} 和 full-sequence 线并列放进同一张表与同一张曲线图。
...
这一小节也回到完整 2 epoch 里按 \texttt{NDCG@10} 选 best 的默认口径。loss 层面的比较只保留三种口径：\texttt{suffix-only GRPO}、\texttt{prefix SFT + suffix-only GRPO}、以及 \texttt{Full-sequence SFT + GRPO}。
...
\midrule
HR@1 & \underline{0.0722} & \textbf{0.0764} & 0.0720 \\
HR@5 & \textbf{0.1014} & \underline{0.1009} & 0.0972 \\
HR@10 & \textbf{0.1189} & \underline{0.1180} & 0.1168 \\
HR@20 & \textbf{0.1447} & \underline{0.1435} & 0.1404 \\
HR@50 & \underline{0.1941} & \textbf{0.1985} & 0.1923 \\
NDCG@5 & \underline{0.0875} & \textbf{0.0890} & 0.0852 \\
NDCG@10 & \underline{0.0931} & \textbf{0.0945} & 0.0914 \\
NDCG@20 & \underline{0.0995} & \textbf{0.1009} & 0.0973 \\
NDCG@50 & \underline{0.1094} & \textbf{0.1118} & 0.1075 \\
\midrule
Selected ckpt & \texttt{checkpoint-2997} & \texttt{checkpoint-1665} & \texttt{checkpoint-999} \\
\bottomrule
\end{tabular}
...
当前 full-sequence 列已经接入 \texttt{results/Instruments-grec-genrec-fixed-full-sequence-sft-from-sft}；当前共同步到 \texttt{3} 个 checkpoint，按 \texttt{NDCG@10} 选出的 readout 是 \texttt{checkpoint-999}。 对应 launcher：\texttt{hope/Instruments-genrec/rl\_fixed\_full\_sequence\_sft.sh}；已完成的 prefix-SFT 列这里使用 \texttt{CE=0.005} 作为代表性 fixed+CE readout。
...
\midrule
HR@1 & \underline{0.0730} & 0.0728 & 0.0720 & \textbf{0.0734} & 0.0702 \\
HR@5 & \underline{0.1041} & 0.1035 & \textbf{0.1048} & 0.1025 & 0.0974 \\
HR@10 & \underline{0.1248} & 0.1239 & \textbf{0.1257} & 0.1216 & 0.1158 \\
HR@20 & 0.1504 & \underline{0.1513} & \textbf{0.1518} & 0.1496 & 0.1405 \\
HR@50 & \underline{0.2007} & \textbf{0.2015} & 0.2006 & 0.1978 & 0.1861 \\
NDCG@5 & \textbf{0.0890} & 0.0885 & \underline{0.0889} & 0.0884 & 0.0843 \\
NDCG@10 & \textbf{0.0956} & \underline{0.0951} & \textbf{0.0956} & 0.0946 & 0.0902 \\
NDCG@20 & \textbf{0.1021} & \underline{0.1020} & \textbf{0.1021} & 0.1016 & 0.0964 \\
NDCG@50 & \textbf{0.1120} & \underline{0.1119} & 0.1118 & 0.1111 & 0.1054 \\
\midrule
Selected ckpt & \texttt{checkpoint-3368} & \texttt{checkpoint-4206} & \texttt{checkpoint-2105} & \texttt{checkpoint-4206} & \texttt{checkpoint-2105} \\
\bottomrule
\end{tabular}%
}
