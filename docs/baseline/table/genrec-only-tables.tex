...
\subsection{Hint Strategy 2x2}

这里把当前最核心的四条 prefix-hint strategy 压缩成一个 \texttt{2x2}。四条线分别是 \texttt{Ours(3-task)}、\texttt{Adaptive hinting}、\texttt{Fixed first-token hint} 和 \texttt{Adaptive first-token hint}。
...
\begin{tabular}{lcccc}
\toprule
Metric & Ours (3-task) & Adaptive hinting & Fixed first-token hint & Adaptive first-token hint \\
\midrule
NDCG@10 & 0.0931 & \underline{0.0936} & \textbf{0.0945} & 0.0921 \\
HR@10 & \underline{0.1189} & 0.1169 & \textbf{0.1205} & 0.1155 \\
NDCG@50 & \underline{0.1094} & 0.1083 & \textbf{0.1103} & 0.1065 \\
HR@50 & \textbf{0.1941} & 0.1855 & \underline{0.1935} & 0.1830 \\
\midrule
Selected ckpt & \texttt{checkpoint-2997} & \texttt{checkpoint-2997} & \texttt{checkpoint-2652} & \texttt{checkpoint-2394} \\
\bottomrule
\end{tabular}
\caption{Instruments 上 prefix-hint strategy 的 \texttt{2x2} 对比。}
\label{tab:genrec-only-instruments-prefix-hint-2x2}
\end{table}

\begin{figure}[p]
\centering
\includegraphics[width=\textwidth]{assets/genrec-only-instruments-prefix-hint-2x2-curves.png}
\caption{Instruments 上 prefix-hint strategy 的 \texttt{2x2} 对比。四条线分别对应 \texttt{Ours(3-task)}、\texttt{Adaptive hinting}、\texttt{Fixed first-token hint} 与 \texttt{Adaptive first-token hint}；横轴统一使用 epoch，其中 dynamic / full fixed 按 \texttt{3326 step = 2 epoch} 归一化，两条 first-token 线按 \texttt{2652 step = 2 epoch} 归一化。}
\label{fig:genrec-only-instruments-prefix-hint-2x2-curves}
\end{figure}

\subsection{Max1 Budget Sensitivity}

最后单独看 \texttt{max1}：如果只允许 adaptive hint 最多暴露 1 个 prefix token，它会不会退化回 plain \texttt{rule-only}。
...
\begin{tabular}{lcccc}
\toprule
Metric & Rule-only baseline & Adaptive hinting & Adaptive hinting max1 & Fixed first-token hint \\
\midrule
NDCG@10 & \textbf{0.0960} & 0.0936 & 0.0934 & \underline{0.0945} \\
HR@10 & \underline{0.1179} & 0.1169 & 0.1158 & \textbf{0.1205} \\
NDCG@50 & 0.1070 & 0.1083 & \underline{0.1095} & \textbf{0.1103} \\
HR@50 & 0.1681 & 0.1855 & \underline{0.1905} & \textbf{0.1935} \\
\midrule
Selected ckpt & \texttt{checkpoint-2997} & \texttt{checkpoint-2997} & \texttt{checkpoint-1332} & \texttt{checkpoint-2652} \\
\bottomrule
\end{tabular}
\caption{Instruments 上 prefix budget 的 \texttt{max1} 消融。}
\label{tab:genrec-only-instruments-max1-ablation}
\end{table}

\begin{figure}[p]
\centering
\includegraphics[width=\textwidth]{assets/genrec-only-instruments-max1-ablation-curves.png}
\caption{Instruments 上 prefix budget 的 \texttt{max1} 消融。图中比较 \texttt{Rule-only baseline}、\texttt{Adaptive hinting}、\texttt{Adaptive hinting max1} 与 \texttt{Fixed first-token hint}；dynamic 主线按 \texttt{3326 step = 2 epoch} 归一化，\texttt{Fixed first-token hint} 按 \texttt{2652 step = 2 epoch} 归一化。}
\label{fig:genrec-only-instruments-max1-ablation-curves}
\end{figure}
...
\caption{Instruments 上三个 fixed-hint taskfix 变体的 headline checkpoint 曲线。图中展示 \texttt{NDCG@10}、\texttt{HR@10}、\texttt{NDCG@50} 与 \texttt{HR@50}；横轴统一使用 epoch，并分别按 \texttt{3326 / 2652 / 3012 step = 2 epoch} 归一化；虚线表示 \texttt{GenRec(sft)} 的整体 best（\texttt{checkpoint-495}）。}
...
\setlength{\tabcolsep}{4pt}
\renewcommand{\arraystretch}{1.08}
\begin{tabular}{p{3.4cm} p{2.4cm} c c p{4.6cm}}
\toprule
Loss design & Instruments readout & Best NDCG@10 & Best HR@50 & Status / note \\
\midrule
suffix-only GRPO & \texttt{GenRec(fixed)} & 0.0931 & 0.1941 & non-CE fixed baseline \\
prefix SFT + suffix-only GRPO & \texttt{fixed+CE} family & 0.0953 & 0.1985 & completed; top-10 best currently comes from \texttt{CE=0.01}, coverage best from \texttt{CE=0.005} \\
full-sequence SFT + GRPO & \texttt{running} & \textemdash & \textemdash & 当前仍在跑，稳定结果尚未回填 \\
\bottomrule
\end{tabular}
...
当前 running 的 full-sequence 线对应 launcher：\texttt{hope/Instruments-genrec/rl\_fixed\_full\_sequence\_sft.sh}。
