...
# Step-by-step runner for Games preprocessing on remote training machine.
# Modes:
...
INDEX_BASENAME_DEFAULT="Games.index_emb-qwen3-embedding-4B_rq4_cb256-256-256-256_dsGames.json"
...
DATASET_KEY_PREFIX="${DATASET_KEY_PREFIX:-Games}"

usage() {
  cat <<EOF
Usage:
  bash scripts/run_games_preprocess.sh [check|prepare|build|all]

Examples:
  bash scripts/run_games_preprocess.sh check
  bash scripts/run_games_preprocess.sh prepare
  bash scripts/run_games_preprocess.sh build
  bash scripts/run_games_preprocess.sh all

Override env vars if needed:
  GENREC_ROOT DATA_ROOT CATEGORY INDEX_PATH OUTPUT_DIR PYTHON_BIN SEQ_SAMPLE SEED
  TASK3_SAMPLE SID_LEVELS SPLIT_STRATEGY HISTORY_MAX TRAIN_ROW_ORDER TRAIN_RATIO VALID_RATIO
...
print_config() {
  ...
  echo "[INFO] TASK3_SAMPLE=${TASK3_SAMPLE}"
  echo "[INFO] SEED=${SEED}"
  echo "[INFO] SID_LEVELS=${SID_LEVELS}"
  echo "[INFO] SPLIT_STRATEGY=${SPLIT_STRATEGY}"
  echo "[INFO] HISTORY_MAX=${HISTORY_MAX}"
  echo "[INFO] TRAIN_ROW_ORDER=${TRAIN_ROW_ORDER}"
